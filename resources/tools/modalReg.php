                ...
                <div class="container-fluid ">
                    <div class="row justify-content-around ">
                        <form class="formulario col-10 col-md-7" action="resources/controllers/formManagement.php" method="post">
                            ...
                            if(isset($_COOKIE['regTipoUser'])){
                                if($_COOKIE['regTipoUser']==2){
                                ?>
                                    <div class="mb-3">
                                        <label for="birthday" class="form-label">Fecha de nacimiento</label>
                                        <input type="date" class="form-control" id="birthday" required name="birthday" value="<?php echo (isset($_COOKIE['regBirthday'])) ? $_COOKIE['regBirthday']:'';?>">
                                    </div>
                                    <div class="mb-3">
                                        <label for="titulation" class="form-label">Titulación</label>
                                        <input type="text" class="form-control" id="titulation"  required name="titulation" value="<?php echo (isset($_COOKIE['regTitulation'])) ? $_COOKIE['regTitulation']:'';?>">
                                    </div>

                                    <div class="mb-3">
                                        <label for="numss" class="form-label">Número de la seguridad Social</label>
                                        <input type="tel" class="form-control" id="inputTelefono" required name="ssNumber" value="<?php echo (isset($_COOKIE['regSS'])) ? $_COOKIE['regSS']:'';?>">
                                    </div>
                                <?php
                                }
                            }
